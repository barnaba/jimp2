...
int read_segments(FILE *infile, triangulateio *input, int markers, int firstnode){
  ...
  for (i = 0; i < input->numberofsegments; i++) {
    ...
    current_segment[0] = (int) strtol(stringptr, &stringptr, 0) - firstnode;

    stringptr = findfield(stringptr);
    if (*stringptr == '\0') {
      fprintf(stderr, "Error:  Segment %d is missing its second endpoint.\n", i);
      return FORMAT_ERR;
    }
    current_segment[1] = (int) strtol(stringptr, &stringptr, 0) - firstnode;
